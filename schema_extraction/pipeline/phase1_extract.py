    ...
    for repo in repos:
        ...
        try:
            ...
            for issue in issues:
                ...
                file_path = repo_dir / f"{item_type}_{issue_number}.json"
                if file_path.exists():
                    print(f"  Skipping {item_type.capitalize()} #{issue_number} (already exists)")
                    continue
                
                print(f"  Processing {item_type.capitalize()} #{issue_number}: {issue['title']}")
                import time
                time.sleep(0.5) # Sleep to avoid secondary rate limits
                # Fetch comments
                try:
                    comments_data = fetch_comments(repo, issue_number, token)
